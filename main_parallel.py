...
def _require_recovery_override(args: argparse.Namespace) -> bool:
    if getattr(args, "explicit_recovery_override", False):
        return True
    print(
        "[ERROR] Bulk crawling is disabled during recovery. "
        "Use --explicit-recovery-override with a non-canonical output root.",
        file=sys.stderr,
    )
    return False
...
def cmd_scan(args: argparse.Namespace) -> int:
    ...
    if not _require_recovery_override(args):
        return 2
    print("=" * 60)
...
def main() -> int:
    ...
    p_scan.add_argument(
        "--download-workers",
        type=int,
        default=SAFE_16GB.max_download_workers,
    )
    ...
    p_scan.add_argument(
        ...
        action="store_true",
        help="Run a limited experimental/non-canonical recovery crawl",
    )
    p_scan.add_argument("--fresh", action="store_true", help="Ignore existing checkpoint")
    ...
    p_resume.add_argument("--checkpoint-dir", type=str, default="data/checkpoints")
    p_resume.add_argument(
        "--output-dir",
        type=str,
        default="data/runtime/experimental/crawler/results",
    )
    p_resume.add_argument(
        "--explicit-recovery-override",
        action="store_true",
        help="Resume a limited experimental/non-canonical recovery crawl",
    )

    # ---- status ----
    p_status = sub.add_parser("status", help="Show checkpoint status")
    p_status.add_argument("--checkpoint-dir", type=str, default="data/checkpoints")

    args = parser.parse_args()

    # Auto-detect workers
    if hasattr(args, "workers") and args.workers is None:
        args.workers = SAFE_16GB.max_analysis_workers
