...
import type { JobListItem, OpsMetrics, Pocket } from '../types/api';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.pockets({ limit: 25, offset: 0, tier: 'high' }, controller.signal),
      api.jobs(controller.signal),
      api.opsMetrics(controller.signal).catch(() => null),
    ])
      .then(([overviewRes, pocketsRes, jobsRes, ops]) => {
        ...
        setStats({
          ...
          heuristic_shortlist_pockets: s.heuristic_shortlist_pockets ?? 0,
          avg_bio_score: (s.avg_bio_score ?? 0).toFixed(3),
        });
        setPockets(pocketsRes.items);
        setJobs((jobsRes.jobs ?? []).slice(0, 10));
        setOpsMetrics(ops);
      })
      .catch((error: unknown) => {
        if (!cancelled) {
          setError(errorMessage(error, 'Failed to load dashboard data'));
  ...
  pockets.forEach((p) => {
    const c = p.heuristic_quality_tier || 'low';
    ...
    return (
      <div>
        ...
        <ErrorNotice
          ...
          detail="Ensure the backend is running at http://127.0.0.1:8000"
        />
      </div>
    );
  }
  ...
  return (
    <div style={{ animation: 'fadeUp 0.3s ease both' }}>
      ...
      >
        Operational view only: heuristic rankings are not benchmark validation or a scientific claim.
      </div>
      <div
        style={{
          display: 'grid',
      ...
      >
        ...
        <KpiCard label="High-tier" value={stats.heuristic_shortlist_pockets ?? 0} icon="H" />
            ...
            data={[
              {
                ...
                type: 'scatter',
                marker: {
                  color: pockets.map((p) => cc[p.heuristic_quality_tier ?? 'low'] || '#6a6a8a'),
                  size: 9,
                  opacity: 0.8,
        ...
        >
          <Plot
            ...
            layout={{
              ...
              title: { ...PLOT_THEME.title, text: 'Heuristic Quality Tier' },
              showlegend: false,
              height: 250,
            }}
            config={{ responsive: true, displayModeBar: false }}
            ...
            data={[
              {
                ...
                type: 'bar',
                marker: {
                  color: top10ByScore.map((p) => cc[p.heuristic_quality_tier ?? 'low'] || '#6a6a8a'),
                  opacity: 0.9,
                },
              },
            ]}
                  ...
                  <td style={{ padding: '6px 8px' }}>
                    <span
                      style={{
                        ...
                        fontWeight: 600,
                        background:
                          p.heuristic_quality_tier === 'high'
                            ? 'rgba(0,255,136,.15)'
                            : p.heuristic_quality_tier === 'medium'
                              ? 'rgba(255,170,34,.15)'
                              : 'rgba(255,68,85,.12)',
                        color:
                          p.heuristic_quality_tier === 'high'
