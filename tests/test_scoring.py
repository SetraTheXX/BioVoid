...
def make_cavity(
    ...
    volume: float = 500.0,
    radius_geom: float = 5.0,
    radius_clear: float = 4.0,
    hydrophobic_ratio: float = 0.6,
    polar_atoms: int = 1,
    druggable: bool = True,
    n_vertices: int = 6,
) -> dict[str, Any]:
    ...
    vertices: list[np.ndarray] = [
        np.array(center) + np.random.randn(3) * 2.0 for _ in range(n_vertices)
    ]
    return {
...
class TestScoringProfiles:
    ...
    def test_profile_score_clamps_values(self):
        ...
        profile = DefaultProfile()
        metrics = {"volume": 2.0, "hydrophobicity": -0.5, "enclosure": 0.5, "depth": 0.5}
        score = profile.calculate_score(metrics)
        # 2.0 → 1.0, -0.5 → 0.0
...
class TestCalculateDepth:
    ...
    def test_depth_bounded(self):
        ...
        atom_coords = make_atom_coords(100)
        for _ in range(20):
            cavity = make_cavity(center=np.random.randn(3) * 30.0)
...
class TestBioScore:
    ...
    def test_missing_geometry_is_not_treated_as_a_valid_shortlist(self):
        cavity = {
            "center": None,
            "volume": 500.0,
        ...
        result = calculate_bio_score(cavity, make_atom_coords())

        assert result["measurement_quality"]["tier"] == "insufficient"
        assert result["score_status"] == "unavailable_due_to_measurement_quality"
        assert result["heuristic_shortlist"] is False
