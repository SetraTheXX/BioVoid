...
def _resume_args(
    *,
    ...
    input_path: str | None,
    checkpoint_dir: str,
    explicit_recovery_override: bool = True,
    ...
    return SimpleNamespace(
        ...
        db=db,
        explicit_recovery_override=explicit_recovery_override,
        output_dir=output_dir or str(Path(checkpoint_dir).parent / "experimental"),
    )
...
def test_resume_is_blocked_without_recovery_override(
    tmp_path: Path,
    ...
    args = _resume_args(
        ...
        explicit_recovery_override=False,
    )
    rc = main_parallel.cmd_resume(args)
    captured = capsys.readouterr()
    assert rc == 2
    assert "Bulk crawling is disabled during recovery" in captured.err
...
def test_scan_cli_defaults_follow_safe_16gb_profile(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    observed: dict[str, int] = {}

    def capture(args):
        observed["workers"] = args.workers
        observed["download_workers"] = args.download_workers
        return 0

    monkeypatch.setattr(main_parallel, "cmd_scan", capture)
    monkeypatch.setattr(
        sys,
        "argv",
        ["main_parallel.py", "scan", "--input", "ids.json"],
    )

    assert main_parallel.main() == 0
    assert observed == {
        "workers": SAFE_16GB.max_analysis_workers,
        "download_workers": SAFE_16GB.max_download_workers,
    }
...
def test_resume_writes_to_db_with_db_flag(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    output_dir.mkdir()
    db_path = output_dir / "crawler.sqlite"
    db_path.touch()
