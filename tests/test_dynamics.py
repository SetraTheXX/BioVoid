...
def test_math_identity():
    ...
    hessian_orig = test_nma_math.build_anm_hessian(coords_orig, cutoff=DEFAULT_CUTOFF, gamma=DEFAULT_GAMMA)
    eigenvalues_orig, eigenvectors_orig = test_nma_math.calculate_normal_modes(hessian_orig, n_modes=10)
    ...
    assert np.allclose(hessian_orig, hessian_new, atol=1e-12), f"❌ Hessian differs! (max diff: {hessian_diff})"
    ...
    assert np.allclose(eigenvalues_orig, eigenvalues_new, atol=1e-10), f"❌ Eigenvalues differ! (max diff: {eigenvalue_diff})"
    ...
    print(f"   ✅ Eigenvectors: IDENTICAL (all parallel)")
...
def test_hessian_validation():
    ...
    print(f"   ✅ validate_hessian() passed")
...
def test_trivial_modes():
    ...
    print(f"   ✅ First 6 modes are trivial (translation + rotation)")
    
    # Validation function
    validate_trivial_modes(hessian)
    print(f"   ✅ validate_trivial_modes() passed")
...
def test_eigenvalue_properties():
    ...
    print(f"   ✅ Frequency range reasonable (0.5-5.0)")
    
    # Validation function
    validate_eigenvalues(eigenvalues)
    print(f"   ✅ validate_eigenvalues() passed")
...
def test_conformation_generation():
    ...
    assert len(conformations) == expected, f"Wrong count: {len(conformations)} (expected {expected})"
...
def test_file_output():
    ...
    print(f"   ✅ Files are valid PDB format")
    
    # File names correct
    assert saved_files[0].name == "frame_001.pdb"
    print(f"   ✅ File naming correct (frame_001.pdb, ...)")
