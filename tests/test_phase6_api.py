...
def test_root_and_dashboard_use_canonical_react_entry() -> None:
    with _build_client() as client:
        root = client.get("/", follow_redirects=False)
        dashboard = client.get("/dashboard", follow_redirects=False)
        assert root.status_code in {200, 503}
        assert dashboard.status_code in {200, 503}
        assert root.headers["X-BioVoid-UI"] in {
            "react-canonical",
            "react-build-required",
        }
        assert dashboard.headers["X-BioVoid-UI"] == root.headers["X-BioVoid-UI"]
...
def test_full_analysis_rejects_non_rcsb_length_before_queueing() -> None:
    with _build_client() as client:
        response = client.post(
            "/jobs",
            headers={"Idempotency-Key": "audit-rcsb-length"},
            json={
...
def test_batch_rejects_invalid_identifier_without_internal_server_error() -> None:
    with _build_client() as client:
        response = client.post(
            "/jobs/batch",
            headers={"Idempotency-Key": "audit-batch-length"},
            json={
                "job_type": "full_analysis",
                "pdb_ids": ["ABCDEFGHIJKLM"],
            },
        )
        assert response.status_code == 400
        assert response.json()["error"]["code"] == "INVALID_BATCH_REQUEST"


def test_batch_full_analysis_rejects_automatic_retries_before_queueing() -> None:
    with _build_client() as client:
        response = client.post(
            "/jobs/batch",
            headers={"Idempotency-Key": "audit-batch-retries"},
            json={
                "job_type": "full_analysis",
                "pdb_ids": ["1CBS"],
...
def test_publication_figure_export_is_quarantined() -> None:
    with _build_client() as client:
        response = client.get("/publication/figure-data")
        assert response.status_code == 200
        payload = response.json()
        assert payload["available"] is False
        assert payload["canonical_eligible"] is False
        assert payload["validation_status"] == "not_eligible"
