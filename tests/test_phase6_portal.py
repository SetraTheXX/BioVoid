...
import src.api.portal as portal_module
...
def test_missing_portal_template_uses_the_same_csp_contract(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(portal_module, "_TEMPLATE_DIR", tmp_path)
    monkeypatch.setattr(portal_module, "_PORTAL_CACHE", None)
    monkeypatch.setattr(portal_module, "_PORTAL_MTIME", 0.0)

    html = portal_module.render_portal_html()
...
def test_job_result_redacts_local_paths() -> None:
    with _build_client() as client:

        def runner(_: JobSubmitRequest) -> dict:
            return {
                "pdb_id": "1CBS",
                "run_workspace": r"D:\private\run",
                "preparation": {
                    "prepared_structure_path": r"D:\private\prepared.pdb",
                    "status": "accepted",
                },
                "nested": {"path": "/" + "home/researcher/results.json", "value": 1},
            }

        client.app.state.orchestrator.register_runner("quick_probe", runner)
        create = client.post(
            "/jobs",
            headers={"Idempotency-Key": "portal-redaction-1"},
            json={
                "job_type": "quick_probe",
                "input": {"pdb_id": "1CBS"},
        ...
        job_id = create.json()["job_id"]
        final = _wait_terminal(client, job_id)
        assert "run_workspace" not in final["result"]
        assert "prepared_structure_path" not in final["result"]["preparation"]
        assert "path" not in final["result"]["nested"]
        assert "private" not in str(final)
        assert "/" + "home/" not in str(final)

        downloaded = client.get(f"/jobs/{job_id}/result")
        assert downloaded.status_code == 200
        assert "run_workspace" not in downloaded.text
        assert "D:\\private" not in downloaded.text
