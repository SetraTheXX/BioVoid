...
def test_grid_box():
    ...
    pockets = [
        {
            ...
            "center": [10, 20, 30],
            "radius_geom": 3.0,
            "expected_size": GRID_MIN_SIZE,
            "label": "Küçük cep (r=3.0)",
        },
        {
            "id": 2,
            "center": [20, 28, 12],
        ...
        },
        {
            ...
            "radius_geom": 10.0,
            "expected_size": 26.0,
            "label": "Orta cep (r=10.0)",
        },
        {
            "id": 4,
            "center": [50, 50, 50],
            "radius_geom": 12.0,
            "expected_size": GRID_MAX_SIZE,
            "label": "Büyük cep (r=12.0)",
        },
        {
            "id": 5,
            "center": [5, 15, 25],
    ...
    for p in pockets:
        ...
        passed = grid.size_x == expected and grid.size_y == expected and grid.size_z == expected
        results.append(
            check(p["label"], passed, f"boyut={grid.size_x:.1f}, beklenen={expected:.1f}")
        )
...
def test_known_ligand_docking():
    ...
    results.append(check("Pozlar bulundu", n_poses >= 1, f"{n_poses} poz"))
...
def test_interaction_analysis():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        results.append(
            check("H-bond: N-O at 3.0 A", report.n_hbonds >= 1, f"{report.n_hbonds} H-bond bulundu")
        )
...
def test_nma_frame_docking():
    ...
    results.append(check("Cep tutarlılığı (≥60%)", consistency >= 0.6, f"{consistency:.0%}"))
...
def main():
    ...
    summary = {
        "phase": 4,
        "results": {
            k: ("passed" if v is True else "failed" if v is False else "skipped")
