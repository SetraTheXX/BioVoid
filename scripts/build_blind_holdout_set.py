...
def main() -> int:
    ...
    parser.add_argument(
        "--allow-legacy-contaminated",
        action="store_true",
        help="Explicitly reproduce the historical contaminated holdout only",
    )
    args = parser.parse_args()
    if not args.allow_legacy_contaminated:
        print(
            "[DISABLED] Historical holdout builder is contaminated and cannot "
            "create the canonical Phase 6 manifest."
        )
        return 2
