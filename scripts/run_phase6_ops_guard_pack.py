"""Run the deprecated artifact-backed Phase 6 guard with explicit opt-in."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Opt in to ignored legacy reports; this is not a public CI gate.",
    )
...
def main() -> int:
    args = parse_args()
    if not args.dry_run and not args.allow_legacy_artifacts:
        print(
            "[ERROR] Legacy artifact guard is disabled by default. "
            "Use the hermetic phase6-contract-guard tests for release checks."
        )
        return 2
