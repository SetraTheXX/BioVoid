...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--allow-legacy-atlas-write",
        action="store_true",
        help="Tarihsel Atlas center repair işlemini açıkça etkinleştir",
    )
...
def main() -> int:
    args = parse_args()
    if not args.allow_legacy_atlas_write:
        print(
            "[DISABLED] Legacy Atlas center repair is excluded from the "
            "canonical Phase 6 workflow."
        )
        return 2
