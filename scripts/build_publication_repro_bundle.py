...
def _bundle_exit_code(missing_count: int) -> int:
    """Return failure when the requested handoff bundle is incomplete."""
...
def main() -> int:
    ...
    lines = [
        ...
        "- Bundle path: `artifacts/publication_repro_bundle_v1/`",
    ...
    status = "DONE" if n_missing == 0 else "FAIL"
    print(f"\n[{status}] Publication repro bundle: {n_copied} copied, {n_missing} missing.")
    return _bundle_exit_code(n_missing)
