"""Run canonical static-v1 on PocketMiner validation and temporal/test apo rows.

The pre-sealed four-case held-out preparation is target-blind. This command
retains the complete final pocket list, keeps the selected shadow ranking out
of detector generation, and never opens holo labels or evaluator data.
"""
...
class PocketMinerHoldoutStaticError(RuntimeError):
    """Raised when the target-blind held-out static run is invalid."""
...
def _read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise PocketMinerHoldoutStaticError(f"cannot read JSON: {path}") from exc
    if not isinstance(value, dict):
        raise PocketMinerHoldoutStaticError(f"JSON must be an object: {path}")
    return value
...
def _validate_manifest(manifest: Mapping[str, Any]) -> None:
    try:
        ...
        raise PocketMinerHoldoutStaticError(str(exc)) from exc
    ...
    if any(token in serialized for token in ("holo", "ligand", "evaluator", "ground_truth")):
        raise PocketMinerHoldoutStaticError("detector manifest contains evaluator data")


def _preflight_index(preflight: Mapping[str, Any]) -> dict[str, Mapping[str, Any]]:
    if preflight.get("status") != "ready_for_static_detector_gate":
        raise PocketMinerHoldoutStaticError("held-out preflight is not ready")
    cases = preflight.get("cases")
    if not isinstance(cases, list):
        raise PocketMinerHoldoutStaticError("held-out preflight cases are missing")
    indexed: dict[str, Mapping[str, Any]] = {}
    for case in cases:
        if not isinstance(case, Mapping) or case.get("status") != "prepared":
            raise PocketMinerHoldoutStaticError("held-out preflight contains blocked cases")
        ...
        if not structure_id or not prepared_path.is_file() or len(prepared_sha256) != 64:
            raise PocketMinerHoldoutStaticError(
                f"prepared held-out input is incomplete: {structure_id}"
            )
        indexed[structure_id] = case
    if len(indexed) != EXPECTED_CASES:
        raise PocketMinerHoldoutStaticError(
            f"held-out static run requires {EXPECTED_CASES} prepared cases"
        )
    return indexed


def run_pocketminer_holdout_static(
    *,
    ...
    prepared = _preflight_index(preflight)
    heldout_cases = [
        case
        for case in manifest.get("cases", [])
        if isinstance(case, Mapping) and case.get("split") in HELDOUT_SPLITS
    ]
    if (
        len(heldout_cases) != EXPECTED_CASES
        or {case.get("split") for case in heldout_cases} != HELDOUT_SPLITS
    ):
        raise PocketMinerHoldoutStaticError("manifest validation/test splits are incomplete")
    if output_path.exists():
        raise PocketMinerHoldoutStaticError(f"refusing to overwrite static output: {output_path}")

    records: list[dict[str, Any]] = []
    for manifest_case in sorted(
        heldout_cases, key=lambda item: (str(item["split"]), str(item["structure_id"]))
    ):
        structure_id = str(manifest_case["structure_id"]).upper()
        preflight_case = prepared.get(structure_id)
        if preflight_case is None:
            raise PocketMinerHoldoutStaticError(f"preflight missing manifest case: {structure_id}")
    ...
    report: dict[str, Any] = {
        "schema_version": "biovoid-pocketminer-heldout-static-v1",
        "status": "completed" if completed == EXPECTED_CASES else "blocked_cases_present",
    ...
    print(f"PocketMiner held-out static: {report['status']} completed={completed}/{EXPECTED_CASES}")
...
def main() -> int:
    args = _parse_args()
    try:
        run_pocketminer_holdout_static(
            manifest_path=args.manifest,
            preflight_path=args.preflight,
            output_path=args.output,
        )
    except (PocketMinerHoldoutStaticError, OSError) as exc:
        print(f"PocketMiner held-out static error: {exc}", file=sys.stderr)
        return 2
    return 0
