...
def _extract_pose_atoms(pdbqt_path: str | Path, pose_index: int = 0) -> list[dict[str, Any]]:
...
def analyze_interactions(
    receptor_pdbqt: str | Path, docked_ligand_pdbqt: str | Path, pose_index: int = 0
) -> InteractionReport:
    """
    ...
    for li, latom in enumerate(lig_atoms):
        ...
        for ri in np.where(dists < VDW_DISTANCE_MAX)[0]:
            ...
            protein_label = (
                f"{ratom['resName']}_{ratom['chainID']}_{ratom['resSeq']}_{ratom['name']}"
            )

            if (
                HBOND_DISTANCE_MIN <= dist <= HBOND_DISTANCE_MAX
