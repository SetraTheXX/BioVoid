...
def validate_known_ligand(
    ...
    if docked_output.exists():
        interaction_report = analyze_interactions(receptor_pdbqt, docked_output, pose_index=0)
...
def dock_nma_frames(
    ...
    if not frames_path.exists():
        return {"error": f"Frames directory not found: {frames_dir}", "success": False}

    frame_files = sorted(frames_path.glob("frame_*.pdb"))
    if not frame_files:
        return {"error": f"No frame_*.pdb files in {frames_dir}", "success": False}
    ...
    if frame_indices is None:
        ...
        if total <= n_sample:
            ...
            selected = [int(i * (total - 1) / (n_sample - 1)) for i in range(n_sample)]
    else:
    ...
    for idx in selected:
        ...
        try:
            receptor_pdbqt = docker.prepare_receptor(
                str(frame_file),
                output_path=str(docker.output_dir / f"{frame_name}_receptor.pdbqt"),
            )
