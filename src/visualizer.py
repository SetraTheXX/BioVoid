...
class BioVoidVisualizer:
    ...
    def create_interactive_view(self, pdb_file: str, cavities: list[dict], pdb_id: str) -> str:
        """
        ...
        for _i, cavity in enumerate(sorted_cavities):
            # Extract points
            ...
            fig.add_trace(
                go.Scatter3d(
                    ...
                    mode="markers",
                    marker=dict(
                        size=size, color=color, opacity=opacity, line=dict(width=border_width)
                    ),
