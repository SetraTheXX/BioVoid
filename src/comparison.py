...
def ensemble_consensus_score(
    ...
    """Compute an experimental ranking score from independent ensemble evidence.

    Combines:
    - bio_score: BioVoid heuristic pocket-quality score
    - Mode support: fraction of accepted modes supporting the pocket
    - Pocket depth and enclosure: from score_components
    - Ensemble support: fraction of accepted independent samples

    Args:
        pocket: Pocket dict with bio_score, score_components, and motion evidence
        total_samples: Accepted samples, for legacy hit-list support calculation
        w_bio: Weight for bio_score (default 0.40)
        w_mode: Weight for mode support (default 0.25)
        w_geometry: Weight for depth+enclosure (default 0.20)
        w_support: Weight for multi-frame support (default 0.15)

    Returns:
        Experimental ranking score in [0, 1].
    """
    components = pocket.get("score_components", {})
    ...
    support_ratio = pocket.get("ensemble_support")
    if support_ratio is not None:
        support = float(support_ratio)
    elif total_samples and total_samples > 0:
        hits = pocket.get("frame_hits", [])
        support = len(hits) / total_samples if isinstance(hits, (list, set)) else 0.0
    else:
        support = 0.0
    support = max(0.0, min(1.0, support))

    mode_support = max(0.0, min(1.0, float(pocket.get("mode_support", 0.0))))

    score = w_bio * bio + w_mode * mode_support + w_geometry * geometry + w_support * support
