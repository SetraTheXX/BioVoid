...
LEGACY_BENCHMARK_PROTOCOL_ID = "legacy_score_weighted_v1"
LEGACY_VALIDATION_STATUS = "legacy_non_validated"


class LegacyBenchmarkDisabledError(RuntimeError):
    """Raised when the quarantined historical evaluator is used implicitly."""
...
class BenchmarkResult:
    ...
    error: str | None = None
    validation_status: str = LEGACY_VALIDATION_STATUS
    canonical_eligible: bool = False
    protocol_id: str = LEGACY_BENCHMARK_PROTOCOL_ID
...
def benchmark_single(
    ...
    *,
    ...
    """
    if not allow_legacy:
        raise LegacyBenchmarkDisabledError(
            "The score-weighted legacy benchmark is disabled during recovery. "
            "Use allow_legacy=True only for explicitly labelled historical reproduction."
        )
...
def compare_with_fpocket(
    ...
    bv_summary = run_benchmark(biovoid_results, known_pockets, tolerance, allow_legacy=True)
    fp_summary = run_benchmark(fpocket_results, known_pockets, tolerance, allow_legacy=True)
