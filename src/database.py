...
DB_VERSION = 1
DEFAULT_DB_PATH = str(PATHS.legacy_atlas_db)
...
class AtlasDB:
    ...
    def __init__(
        ...
        legacy_path = Path(PATHS.legacy_atlas_db).resolve()
        self.read_only = self.db_path.resolve() == legacy_path if read_only is None else read_only
        if self.read_only:
            if not self.db_path.is_file():
                raise FileNotFoundError(f"Read-only legacy Atlas does not exist: {self.db_path}")
        else:
    ...
    def _connect(self) -> None:
        ...
        database = (
            f"{self.db_path.resolve().as_uri()}?mode=ro" if self.read_only else str(self.db_path)
        )
