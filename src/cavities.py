...
Geometric cavity merging, volume calculation, and heuristic pocket-quality analysis.
...
from .geometry import extract_atom_coords, find_voids
...
# Hydrophobic residues (heuristic pocket-quality filter)
...
def merge_cavities(voids: list[dict], merge_threshold: float = MERGE_THRESHOLD) -> list[dict]:
    """
    ...
    try:
        clusters = fclusterdata(centers, t=merge_threshold, criterion="distance", method="complete")
    except Exception:
    ...
    for cid in cluster_ids:
        ...
        cluster_voids = [v for v, m in zip(voids, cluster_mask, strict=False) if m]
...
def filter_hydrophobic(
    cavities: list[CavityDict], pdb_file: str, search_radius: float = HYDROPHOBIC_SEARCH_RADIUS
) -> list[CavityDict]:
    """
    ...
    for cavity in cavities:
        ...
        hydrophobic_count = sum(1 for r in nearby_res_names if r in HYDROPHOBIC_RESIDUES)
