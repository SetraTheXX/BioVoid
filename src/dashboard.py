...
This dashboard has been replaced by the canonical FastAPI/React interface.
Use the local API instead:
...
APP_TITLE = "BioVoid — Pocket Analysis Dashboard (DEPRECATED)"
APP_ICON = "🧬"
PAGE_SIZE = 50
DEFAULT_DB = str(PATHS.legacy_atlas_db)
...
def build_kpi_cards(stats: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        {
            "label": "Heuristic shortlist",
            "value": stats.get(
                "heuristic_shortlist_pockets",
                stats.get("druggable_pockets", 0),
            ),
            "delta": None,
        },
