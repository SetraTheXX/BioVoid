...
def calculate_confidence(cavity: dict[str, Any], metrics: dict[str, float]) -> dict[str, Any]:
    """
    ...
    weighted_score = sum(metrics.get(k, 0) * c for k, c in metric_confs.items()) / sum(
        metric_confs.values()
    )
